...
/// inferred purely from BLE RSSI trend — deliberately compass-free, since
/// magnetometer headings are unreliable indoors and don't assume any
/// particular phone orientation.
enum _RssiTrend { towardNext, awayFromNext, unknown }
...
class NavigationController extends ChangeNotifier {
  ...
  /// Rolling window of recent RSSI readings for the *next* route beacon,
  /// used to infer whether the user is walking toward or away from it —
  /// see [_updateNextBeaconTrend]. Reset whenever the tracked next-beacon
  /// changes (new segment) so a stale trend never leaks across segments.
  final List<double> _nextBeaconRssiWindow = [];
  String? _trackedNextBeaconId;
  _RssiTrend _nextBeaconTrend = _RssiTrend.unknown;

  static const int _rssiTrendWindowSize = 10;

  /// Minimum average RSSI change (dB) between the first and second half of
  /// the window before we trust a direction — below this, readings are
  /// treated as ambiguous/noisy and progress is not blocked. RSSI has
  /// enough natural jitter that a low threshold would cause false
  /// "moving away" calls on flat, noisy signal.
  static const double _rssiTrendThresholdDb = 4;

  /// Cold-start (no confirmed beacon yet) RSSI averaging — see
  /// [_selectInitialFixCandidate]. Kept separate from the steady-state
  /// switching logic above, since the very first fix has none of that
  /// logic's protection (both [_shouldSwitchBeacon] and
  /// [_isPhysicallyReachable] short-circuit to true when there's no anchor
  /// yet to compare against) and is exactly when standing equidistant
  /// between several beacons with noisy, comparable RSSI is most likely.
  ...
  /// Minimum average RSSI lead the best beacon needs over the second-best
  /// before the first fix commits to it. Below this, several beacons are
  /// genuinely comparable — better to keep sampling than to guess.
  static const double _initialFixMinMarginDb = 8;

  /// If no beacon has pulled ahead by [_initialFixMinMarginDb] within this
  /// window, commit to whichever has the best average anyway rather than
  /// waiting indefinitely — logged as a low-confidence pick so it's visible
  /// in telemetry if it happens often.
  ...
  void _onRssiUpdate(Map<String, double> rssiByBleId) {
    ...
    _log("NAV BEACONS candidate is ${candidate?.name}");
    ...
    if (candidate == null && currentBeacon == null && destinationBeacon != null) {
      // Still gathering samples for the cold-start fix — surface this so
      // the UI can show "locating you" instead of an unexplained blank
      // state while _selectInitialFixCandidate waits for a confident lead.
  ...
  Beacon? _selectBeaconCandidate(Map<String, double> rssiByBleId) {
    ...
    if (exploring || currentBeacon == null) {
      // Either the user hasn't picked a destination yet — in which case
      // there's no established route to protect, so keep continuously
      // refining toward the best-guess starting position rather than
      // locking onto a possibly-wrong early pick — or there's no fix at
      // all yet (true cold start). Both use the same noise-resistant
      // averaged, margin-gated comparison instead of a single
      // instantaneous "strongest wins" reading.
      final navigatingStrict = !exploring &&
          currentPath.length > 1 &&
          !allowOffRouteBeacons;
      final eligibleIds = navigatingStrict
          ? currentPath.map((beacon) => beacon.id).toSet()
          : null; // null = no restriction, any visible beacon is eligible
      return _selectInitialFixCandidate(rssiByBleId, eligibleIds);
    }

    final navigating = currentPath.length > 1;
  ...
  /// Picks the initial beacon anchor from an averaged, margin-gated
  /// comparison rather than a single instantaneous strongest reading.
  /// Each visible eligible beacon's RSSI accumulates into a short rolling
  /// window; a candidate is only returned once one beacon's average is
  /// either clearly ahead of the runner-up by [_initialFixMinMarginDb], or
  /// [_initialFixMaxWait] has elapsed with no clear leader (at which point
  /// we commit to the best average anyway rather than stalling forever).
  /// Returns null while still gathering — the caller (an [_onRssiUpdate]
  /// with a null candidate) simply takes no action that tick, which is the
  /// correct behavior: better to wait a few hundred ms for a confident fix
  /// than to lock onto whichever beacon won one noisy reading.
  Beacon? _selectInitialFixCandidate(
    ...
    for (final entry in rssiByBleId.entries) {
      ...
      if (eligibleBeaconIds != null && !eligibleBeaconIds.contains(beacon.id)) continue;
      final window = _initialFixRssiWindows.putIfAbsent(beacon.id, () => []);
      window.add(entry.value);
      if (window.length > _initialFixWindowSize) window.removeAt(0);
    }

    final averages = <String, double>{};
    _initialFixRssiWindows.forEach((beaconId, samples) {
      if (eligibleBeaconIds != null && !eligibleBeaconIds.contains(beaconId)) return;
      if (samples.length < _initialFixMinSamples) return;
      averages[beaconId] = _average(samples);
    });

    if (averages.isEmpty) return null; // still gathering — no fix yet

    final ranked = averages.entries.toList()..sort((a, b) => b.value.compareTo(a.value));
    final bestId = ranked.first.key;
    final bestAvg = ranked.first.value;
    final elapsed = DateTime.now().difference(_initialFixStartedAt!);

    if (ranked.length > 1) {
      final margin = bestAvg - ranked[1].value;
      if (margin < _initialFixMinMarginDb) {
        if (elapsed < _initialFixMaxWait) {
          _log('NAV## initial fix ambiguous — top candidates: '
              '${ranked.take(3).map((e) => '${storeMap.beaconById(e.key)?.name ?? e.key}:${e.value.toStringAsFixed(1)}dB').join(", ")}'
              ' — waiting for a clearer signal (${elapsed.inMilliseconds}ms elapsed)');
          return null;
        }
        _log('NAV## initial fix timeout after ${elapsed.inSeconds}s — committing to best average '
            '(${storeMap.beaconById(bestId)?.name}, margin only ${margin.toStringAsFixed(1)}dB) despite low confidence');
      }
    }
  ...
  bool _isPhysicallyReachable(Beacon candidate) {
    ...
    // NOTE: previously this method also returned true unconditionally when
    // _metersSinceBeacon <= 0 ("hasn't moved yet, let it settle"). That's
    // removed — it created a second bypass window right after every beacon
    // confirmation where literally any candidate would pass regardless of
    // distance. A candidate arriving before any steps have been taken
    // should still be judged: walkingDistanceMeters <= tolerance alone
    // (below) already covers "the very next beacon, confirmed instantly"
    // correctly, without needing a blanket escape hatch.
    ...
    _log('NAV## reachability check: ${candidate.name} is '
        '${walkingDistanceMeters.toStringAsFixed(1)}m away via graph from ${anchor.name}, '
        '${_metersSinceBeacon.toStringAsFixed(1)}m walked so far '
        '(tolerance ${_reachabilityToleranceMeters}m) → ${reachable ? "REACHABLE" : "REJECTED"}');
    return reachable;
  }

  /// Updates [_nextBeaconTrend] from a rolling window of RSSI readings for
  /// the current route's *next* beacon — a compass-free proxy for whether
  /// the user is walking toward it (signal strengthening) or away from it
  /// (signal weakening). Deliberately conservative: with fewer than 4
  /// samples, or when the change is within [_rssiTrendThresholdDb], the
  /// trend is left as [_RssiTrend.unknown] rather than guessing, since RSSI
  /// noise alone can easily produce a spurious few-dB swing.

  //USE this for compass only
  Beacon? headingTowardsBeaconInPath;
  void _updateNextBeaconTrend(Map<String, double> rssiByBleId) {
    final cb = currentBeacon;
    final next = cb == null ? null : _pathBeaconAfter(cb.id);
    if (next == null) {
    ...
    final rssi = _rssiForBeacon(next, rssiByBleId);
      if (rssi == null ) {
        return;
      }

    // Next beacon not currently visible — keep last known trend.

    _nextBeaconRssiWindow.add(rssi);
    if (_nextBeaconRssiWindow.length > _rssiTrendWindowSize) {
      _nextBeaconRssiWindow.removeAt(0);
    }
    if (_nextBeaconRssiWindow.length < 4) return; // Not enough samples yet.

    final mid = _nextBeaconRssiWindow.length ~/ 2;
    final firstAvg = _average(_nextBeaconRssiWindow.sublist(0, mid));
    final secondAvg = _average(_nextBeaconRssiWindow.sublist(mid));
    final delta = secondAvg - firstAvg; // positive = signal strengthening = getting closer
    ...
    if (_nextBeaconTrend == _RssiTrend.towardNext) {
    ...
    if (previousTrend != _nextBeaconTrend) {
      ...
      _log('NAV## RSSI trend toward ${next.name}: $_nextBeaconTrend (Δ${delta.toStringAsFixed(1)}dB)');
    }
  }
  ...
  void _snapToCurrentBeacon() {
    ...
    // The "next beacon" target just changed — any trend gathered for the
    // previous segment's target is meaningless now.
    _nextBeaconRssiWindow.clear();
    _trackedNextBeaconId = null;
    _nextBeaconTrend = _RssiTrend.unknown;
    // First fix has committed — the cold-start averaging state has done its
    // job and won't be consulted again unless currentBeacon becomes null.
  ...
  void _onStep(double distanceMeters) {
    ...
    _log("NAV## steps taken distanceMeters = $distanceMeters and _metersSinceBeacon = $_metersSinceBeacon");
    ...
    // Without a reliable indoor compass, the RSSI trend toward the next
    // route beacon (see _updateNextBeaconTrend) stands in as a direction-
    // of-travel signal. If the signal has been clearly weakening, the user
    // is more likely walking away from the next beacon than toward it, so
    // those steps shouldn't count as forward route progress. The pullback
    // is damped (half weight) rather than a full reversal, since this is a
    // noisy proxy, not a precise measurement — better to slow progress and
    // let it self-correct than to visibly yank the marker backward on a
    // borderline reading.
    final effectiveDelta = _nextBeaconTrend == _RssiTrend.awayFromNext
        ? -distanceMeters * 0.5
        : distanceMeters;

    final segmentProgress = (_segmentProgressMeters + effectiveDelta).clamp(0.0, segment);
  ...
  void _recordCompletedSegmentCalibration(Beacon? previous, Beacon candidate) {
    ...
    _log("NAV## recordStrideCalibration called with ${edge.distanceMeters}");
  ...
  /// True if the user should be allowed to switch from [current] to
  /// [candidate]. Behavior differs by phase:
  ///
  /// - **Exploring** (no destination set yet): there's no established route
  ///   to protect, only a best-guess starting position to keep refining.
  ///   [candidate] already passed [_selectInitialFixCandidate]'s averaged,
  ///   margin-gated comparison before ever reaching here, so no further
  ///   gate is applied — rejecting it again with the reachability/threshold
  ///   checks meant for an in-progress walk would just prevent legitimate
  ///   corrections (e.g. the true nearest beacon losing to a momentarily
  ///   noisy one) while the user hasn't even started moving.
  /// - **Navigating** (destination set): the reachability check runs in
  ///   EVERY branch that can return true — it must never be skippable,
  ///   because a dropped-out reading for the current beacon (currentRssi
  ///   == null, which happens routinely with BLE scans) is exactly the
  ///   moment an implausible candidate is most likely to otherwise slip
  ///   through.
  ...
  //  if (!_isPhysicallyReachable(candidate)) return false;
    ...
    var shouldSwitch =  candidateRssi >= currentRssi + _beaconSwitchThresholdDb;



    _log("NAV BEACONS check :should switch $shouldSwitch to ${current.name} when heading towards ${headingTowardsBeaconInPath?.name}");
    if(shouldSwitch ) {
      
      
      

      _log("NAV## shouldSwitch  = $shouldSwitch for ${candidate.name}");

      if (headingTowardsBeaconInPath?.id == candidate?.id) {
        _log("NAV## shouldSwitch  headingTowardsBeacon also matched = $shouldSwitch for ${candidate.name}");
        
        if(_isPhysicallyReachable(candidate)) {
          _log("NAV## shouldSwitch  _isPhysicallyReachable true for= ${candidate.name}");

          //very confident case
          return true;
        }else{
          _log("NAV## shouldSwitch  NOT _isPhysicallyReachable FALSe for= ${candidate.name}");

          //not much confident case
          return false;
        }
      }
      else {
        
        _log("NAV## shouldSwitch  headingTowardsBeacon NOT matched candidate ${candidate.name} but heading towards ${headingTowardsBeaconInPath?.name}");
        return true;
      }
    }
    else {
